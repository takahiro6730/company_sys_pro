                        ...
                        <ol class="inline-flex items-center space-x-1 ltr:md:space-x-3 rtl:md:space-x-0">
                            ...
                            <li>
                                <div class="flex items-center">
                                    ...
                                    <a href="#"
                                        class="ltr:ml-1 rtl:mr-1 text-sm font-medium text-gray-500 hover:text-gray-900 ltr:md:ml-2 rtl:md:mr-2 dark:text-gray-100 dark:hover:text-white">入金情報一覧</a>
                                </div>
                            </li>
                        </ol>
                    </nav>
                            ...
                            <table id="datatable" class="table w-full pt-4 text-gray-700 dark:text-zinc-100">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td
                                            class="p-4 pr-8 border rtl:border-l-0 border-t-0 border-gray-50 dark:border-zinc-600] text-center">
                                            {{ $deposit->user->nickname }}</td>
                                        <td
                                            class="p-4 pr-8 border border-t-0 border-l-0 border-gray-50 dark:border-zinc-600 text-center">
                                            {{ $deposit->deposited_date }}</td>
                                        <td
                                            class="p-4 pr-8 border border-t-0 border-l-0 border-gray-50 dark:border-zinc-600 text-center">
                                            {{ $deposit->client_info }}</td>
                                        <td
                                            class="p-4 pr-8 border border-t-0 border-l-0 border-gray-50 dark:border-zinc-600 text-center">
                                            {{ $deposit->amount }}</td>
                                        <td
                                            class="p-4 pr-8 border border-t-0 border-l-0 border-gray-50 dark:border-zinc-600 text-center">
                                            {{ $deposit->unit->title }}</td>
                                        <td
                                            class="p-4 pr-8 border border-t-0 border-l-0 border-gray-50 dark:border-zinc-600 text-center">
                                            {{ $deposit->payment->title }}</td>
                                        <td
                                            class="p-4 pr-8 border border-t-0 border-l-0 border-gray-50 dark:border-zinc-600 text-center">
                                            {{ $deposit->other }}</td>
                                        <td
                                            class="p-4 pr-8 border rtl:border-l border-t-0 border-l-0 border-gray-50 dark:border-zinc-600 text-center">
                                            @if ($deposit->register_user == $user_id || Auth::user()->id < 3)
                                                <button onclick="showEditModal('{{ $deposit->id }}', '{{ $deposit->deposited_date }}', '{{ $deposit->client_info }}', '{{ $deposit->amount }}', '{{ $deposit->unit_id }}', '{{ $deposit->payment_id }}', '{{ $deposit->other }}')"
                                        ...
                                        <td
                                            class="p-4 pr-8 border rtl:border-l border-t-0 border-l-0 border-gray-50 dark:border-zinc-600 text-center">
                                            @if ($deposit->register_user == $user_id || Auth::user()->id < 3)
                                                <button id="del_btn[{{ $i }}]" onclick="showDeleteConfirmModal({{ $deposit->id }})"
...
<div class="relative z-50 modal hidden" id="deleteDepositConfirmModal" aria-labelledby="modal-title" role="dialog"
    aria-modal="true">
            ...
            <div
                ...
                <div class="p-5 text-center bg-white dark:bg-zinc-700">
                    ...
                    <div class="justify-center px-4 py-3 mt-5 border-gray-50 sm:flex sm:px-6">
                        ...
                        <button type="button" onclick="deleteDeposit();" class="inline-flex justify-center w-full px-4 py-2 text-base font-medium text-white bg-red-500 border border-transparent rounded-md shadow-sm btn hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm">削除</button>
                    </div>
                </div>
            </div>
        </div>
...
<div class="relative z-50 modal hidden" id="editDepositModal" aria-labelledby="modal-title" role="dialog"
    aria-modal="true">
    <div class="fixed inset-0 z-50 overflow-hidden">
        <div class="absolute inset-0 transition-opacity bg-black bg-opacity-50"></div>
        <div
            ...
            <div
                ...
                <div class="p-5 text-center bg-white dark:bg-zinc-700">
                    ...
                    <h2 class="mt-5 text-xl text-gray-700 dark:text-gray-100">入金情報変更</h2>
                            ...
                            <div class="card-body">
                                <div class="grid grid-cols-12 gap-5 ">
                                    <div class="col-span-12 lg:col-span-12">
                                        ...
                                        <div class="mb-4">
                                            <label class="block font-medium text-gray-700 dark:text-gray-100 mb-2">入金日 *</label>
                                            <input class="w-full rounded border-gray-100 placeholder:text-sm focus:border focus:border-violet-500 focus:ring-0 dark:bg-zinc-700/50 dark:border-zinc-600 dark:placeholder:text-zinc-100 dark:text-zinc-100"
                                            type="date" name="deposited_date" id="deposited_date">
                                            <span id="deposite_date_invalid" class="invalid-feedback hidden" role="alert">
                                                <strong>入金日を入力してください。</strong>
                                            </span>
                                        </div>
                                        <div class="mb-4">
                                            <label class="block font-medium text-gray-700 dark:text-gray-100 mb-2">クライアント名 *</label>
                                            <input class="w-full rounded border-gray-100 placeholder:text-sm focus:border focus:border-violet-500 focus:ring-0 dark:bg-zinc-700/50 dark:border-zinc-600 dark:placeholder:text-zinc-100 dark:text-zinc-100"
                                            type="text" name="client_info" id="client_info">
                                            <span id="client_info_invalid" class="invalid-feedback hidden" role="alert">
                                                <strong>クライアント名を入力してください。</strong>
                                            </span>
                                        </div>
                                        <div class="mb-4">
                                        ...
                                        <div class="mb-4">
                                            <label class="block font-medium text-gray-700 dark:text-gray-100 mb-2">入金経路 *</label>
                                            <select class="w-full rounded border-gray-100 placeholder:text-sm focus:border focus:border-violet-500 focus:ring-0 dark:bg-zinc-700/50 dark:border-zinc-600 dark:placeholder:text-zinc-100 dark:text-zinc-100"
                                            ...
                                            <span id="payment_id_invalid" class="invalid-feedback hidden" role="alert">
                                                <strong>入金経路を入力してください。</strong>
                                            </span>
                                        </div>
                                        <div class="mb-4">
                    ...
                    <div class="justify-center px-4 py-3 mt-5 border-gray-50 sm:flex sm:px-6">
                        <button type="button" class="inline-flex justify-center w-full px-4 py-2 mt-3 text-base font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm btn dark:text-gray-100 hover:bg-gray-50/50 focus:outline-none focus:ring-2 focus:ring-gray-500/30 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm dark:bg-zinc-700 dark:border-zinc-600 dark:hover:bg-zinc-600 dark:focus:bg-zinc-600 dark:focus:ring-zinc-700 dark:focus:ring-gray-500/20" data-tw-dismiss="modal">戻る</button>
                        <button type="button" onclick="updateDeposit();" class="inline-flex justify-center w-full px-4 py-2 text-base font-medium text-white bg-green-500 border border-transparent rounded-md shadow-sm btn hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm">変更</button>
                    </div>
                </div>
            </div>
        </div>
